...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler bubbleRefresh:(BOOL) isBubbled{
    SVInfiniteScrollingView *view;
    if(!self.infiniteScrollingView) {
        if (isBubbled) {
            
            view = [[SVInfiniteScrollingView alloc] initWithBubbledFrame:CGRectMake(0, self.contentSize.height, self.bounds.size.width, SVInfiniteScrollingViewHeight)];
...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    ...
    [self addInfiniteScrollingWithActionHandler:actionHandler bubbleRefresh:YES] ;
}
...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler spin:(RTSpinKitViewStyle)style
{
    if(!self.infiniteScrollingView) {
        ...
        SVInfiniteScrollingView *view = [[SVInfiniteScrollingView alloc] initWithFrame:CGRectMake(0, self.contentSize.height, self.bounds.size.width, SVInfiniteScrollingViewHeight) style:style];
...
- (id)initWithBubbledFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        self.isBubbleRefresh=YES;
        // default styling values
        self.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray;
        self.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        self.state = SVInfiniteScrollingStateStopped;
        self.enabled = YES;
        self.refreshView = [[SCBubbleRefreshView alloc] initWithFrame:(CGRect){0, 0, kScreenWidth, SVInfiniteScrollingViewHeight}];
...
- (id)initWithFrame:(CGRect)frame style:(RTSpinKitViewStyle)style {
    if(self = [super initWithFrame:frame]) {
        self.isSpinRefresh = YES;
        
        self.spinView = [[NaRTSpinKitView alloc] initWithStyle:style];
        self.spinView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin|UIViewAutoresizingFlexibleRightMargin;
        [self addSubview:self.spinView];
        
        self.spinView.center = CGPointMake(CGRectGetWidth(self.frame)/2, CGRectGetHeight(self.frame)/2);
        // default styling values
        self.activityIndicatorViewStyle = UIActivityIndicatorViewStyleGray;
        self.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        self.state = SVInfiniteScrollingStateStopped;
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
        ...
        NSLog(@"scrollOffsetThreshold = %f  contentOffset.y  = %f",scrollOffsetThreshold,contentOffset.y );
        if(!self.scrollView.isDragging && self.state == SVInfiniteScrollingStateTriggered)
            self.state = SVInfiniteScrollingStateLoading;
        else if(contentOffset.y > ABS(scrollOffsetThreshold)  && self.state == SVInfiniteScrollingStateStopped && self.scrollView.isDragging)
            self.state = SVInfiniteScrollingStateTriggered;
        else if(contentOffset.y < ABS(scrollOffsetThreshold)  && self.state != SVInfiniteScrollingStateStopped)
            self.state = SVInfiniteScrollingStateStopped;
    }
}
...
- (UIActivityIndicatorView *)activityIndicatorView {
    if(!_activityIndicatorView) {
        if(self.isBubbleRefresh || self.isSpinRefresh){
            return nil;
        }
